...
export class MarketplaceService {
  ...
  getIdleItem(id: number) {
    return this.idleItems.find(i => i.id === id);
  }

  getMaterialRequest(id: number) {
    return this.materialRequests.find(r => r.id === id);
  }

  publishIdleItem(publishDto: any) {
    const newItem: IdleItem = {
  ...
  publishMaterialRequest(publishDto: any) {
